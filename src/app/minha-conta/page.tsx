...
export default function ProfilePage() {
  ...
  return (
    <div className="bg-white border border-zinc-100 rounded-xl p-6 shadow-sm">
      <h2 className="font-semibold text-lg text-zinc-900 mb-6">Meu Perfil</h2>
      ...
      <form onSubmit={handleSave} className="space-y-4">
        ...
        <h3 className="text-sm font-semibold text-zinc-900 pt-4">Endereço</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="sm:col-span-2">
            <label className="block text-sm font-medium text-zinc-700 mb-1.5">Rua</label>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-zinc-700 mb-1.5">Estado</label>
            <input type="text" value={address.estado} onChange={(e) => setAddress((p) => ({ ...p, estado: e.target.value }))} className={inputClass} maxLength={2} />
          </div>
        </div>

        {message && (
          <p className={`text-sm px-3 py-2 rounded-lg ${message.includes("sucesso") ? "text-green-700 bg-green-50" : "text-red-600 bg-red-50"}`}>
            {message}
          </p>
        )}
        ...
        <button
          type="submit"
          disabled={saving}
          className="flex items-center justify-center gap-2 bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white font-semibold text-sm px-6 py-3.5 min-h-[48px] transition-colors duration-200 cursor-pointer rounded-xl"
        >
